...
document.querySelectorAll('.cdp-size-picker button').forEach((button) => button.addEventListener('click', () => { selectedSize = button.dataset.size; document.querySelectorAll('.cdp-size-picker button').forEach((item) => item.classList.toggle('selected', item === button)); }));
document.querySelector('#cdp-add-to-bag').addEventListener('click', () => { if (!selectedSize) { showToast('Select a size first.'); return; } cart.push({ ...product, size: selectedSize }); renderCart(); openCart(); showToast('Added to your bag.'); });
